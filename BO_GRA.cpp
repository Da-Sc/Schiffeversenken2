...
void BO_GRA::spielfeldAusgabe(char* tmpFeldchar)
{
    ...
    for(int i=0; i<200; i++)
    {
        ...
        if(letzteSpielfeldausgabe[i]!='-')
        {
            ...
            default:
                int tmp=0;
                if(letzteSpielfeldausgabe[i]>='A' && letzteSpielfeldausgabe[i]<='N')
                {
                    tmp = int(letzteSpielfeldausgabe[i])-65;
                    if(tmp>27) exit(-3);
                    einzelFeld=einzelFeldSchiffteile[tmp];
                }
                else if(letzteSpielfeldausgabe[i]>='a' && letzteSpielfeldausgabe[i]<='n')
                {
                    tmp = int(letzteSpielfeldausgabe[i])-97+14;
                    if(tmp>27) exit(-3);
                    einzelFeld=einzelFeldSchiffteile[tmp];
                }
                else einzelFeld=einzelFeldWasser;
                break;
            }
...
bool BO_GRA::schiffsetzen(int tmpSpieler, int tmpLaenge, POSITION *tmpAnfang, POSITION *tmpEnde)
{
    ...
    while(!grosseSchleifeBeenden)
    {
        ...
        while(!schleifeBeenden)
        {
            ...
            switch(ereignis.type)
            {
            case SDL_MOUSEMOTION://verschiebe Bild des Schiffes
                if(aufSchiffhorizontal)
                {
                    erneuereGraphischeOberflaeche(true, rectSchiffhorizontal);
                    if(tmpLaenge%2!=0)
                ...
                break;
            case SDL_MOUSEBUTTONUP:
            case SDL_MOUSEBUTTONDOWN://versuche das Schiff zu setzen
                tmpX=ereignis.button.x;
                tmpY=ereignis.button.y;
                //wurde die maus seit dem letzten maustasten ereignis bewegt?
